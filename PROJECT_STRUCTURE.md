...
- `src/App.vue`  
  ...
  选人页加点模块已移动到左侧标题区下方；右侧保留空白占位列以维持中间主区居中。
  选人页角色分页展示数量已接入宽度阈值：窗口宽度 `<= 1600` 时每页展示 3 名，否则每页展示 6 名。
  加点弹窗支持按角色切换、属性 `+1/-1`、单角色重置（返还该角色已用点数）。
  战斗页“结束游戏并重新开始 / 结束游戏并退出”按钮已移动到左侧“技能攻击力计算公式”模块内，位于连战模式按钮下方，并统一接入“游戏结束”结算弹窗。  
  左侧“技能攻击力计算公式”模块内新增“随机倍率影响”开关（关闭/开启随机倍率影响）。
  “选择招式”模块操作按钮已改为与标题同行横向排列（含图标）。
  “选择招式”模块新增“缩略模式”开关：开启后 4 个出招栏技能卡仅显示技能名称，并以文字按钮形态展示。
  “开启缩略模式”按钮带呼吸动效，并支持悬浮提示其使用场景说明。
...
- `src/components/UnitCard.vue`  
  角色信息卡片，展示属性与状态。  
  角色卡片头部已移除“拥有者”展示；“被动特长”按钮改为右上角入口，悬浮信息面板改为按钮下方展开。  
...
- `src/components/SkillCard.vue`  
  技能卡片，展示技能描述、命中、暴击等。  
  已支持 `compact` 缩略模式：仅显示技能名称，并以自适应宽度文字按钮形式展示。

- `src/components/BattleLog.vue`  
  战斗日志展示与分页控制。  
  字号与间距已做轻量下调，以提升中部区域可视高度。  
...
- 主界面样式响应式改造：核心布局、卡片与弹窗尺寸由固定值迁移为 `clamp/rem/vw/vh` 组合，适配不同分辨率。
- 选人页布局重排：加点模块移动到左侧，右侧保留空白占位，主区保持居中。
- 选择招式区交互升级：标题行横向操作按钮（含图标）+ 缩略模式（技能名文字按钮）。
